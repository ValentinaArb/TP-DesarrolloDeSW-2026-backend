...
export class MedicoService {
  ...
  async orquestradorMedico(
    ...
    if (validado.horaInicio !== undefined) {
      return await turnoService.modificarTurno(
        medicoId,
        turnoId,
        validado.horaInicio,
      );
  ...
  async agregarDisponibilidad(
    ...
    const servicioCompleto = await this.servicioRepository.findById(
      servicio._id ?? servicio.id ?? servicio,
    );
    if (!servicioCompleto) throw new NotFoundError("El servicio no existe");

    const sedeCompleta = await this.sedeRepository.findById(
      sede._id ?? sede.id ?? sede,
    );
    ...
    await agenda.generarTurnosParaNuevaDisponibilidad(
      medico,
      nuevaDisponibilidad,
    );
    medico.agregarDisponibilidad(nuevaDisponibilidad);

    medico.servicios = medico.disponibilidades
      .map((d) => d.servicio)
      .filter(
        (s, i, arr) =>
          s &&
          arr.findIndex(
            (x) => String(x?._id ?? x?.id) === String(s._id ?? s.id),
          ) === i,
      );
    medico.sedes = medico.disponibilidades
      .map((d) => d.sede)
      .filter(
        (s, i, arr) =>
          s &&
  ...
  async eliminarDisponibilidad(idMedico, idDisponibilidad) {
    ...
    const disponibilidad = medico.disponibilidades.find(
    ...
    const agenda = new AgendaService();
    await agenda.eliminarTurnosDisponiblesDeDisponibilidad(
      medico.id,
      disponibilidad,
    );
  ...
  async estaDisponible(medicoId, turno) {
    ...
    return disponibilidadesMedico.some((d) => {
      const dispoConMetodos = new DisponibilidadHoraria(
        null,
        d.diaSemana,
        d.horaDesde,
        d.horaHasta,
      ...
      return (
        dispoConMetodos.abarca(fechaInicio) ||
        dispoConMetodos.abarca(fechaFinal)
      );
    });
  }
  ...
  async yaTieneTurno(medicoId, turnoChequear, turnoService) {
    ...
    return turnosYaDados.some(
      (t) => !turnoService.noSeSuperponen(t, turnoChequear),
    );
  }
  async crearMedico(
    usuario,
    ...
    try {
      const medicoExistente =
        await this.medicoRepository.findByMatricula(matricula);
      if (medicoExistente) {
  ...
  async modificarDisponibilidad(
    medicoId,
    disponibilidadAModificarId,
    diaSemana,
    horaDesde,
    horaHasta,
    servicio,
    ...
    const disponibilidadNueva = new DisponibilidadHoraria(
      null,
      diaSemana,
      horaDesde,
      horaHasta,
  ...
  async marcarTurnoComo(turnoId, estado) {
    ...
    if (estado === "CANCELADO") {
      ...
      await turno.actualizarEstado(
        estado,
        turno.paciente,
        `El médico marcó el turno como ${estado}`,
      );
    }
  ...
  async consultarHistorialTurnos(pacienteId, medicoId, estado) {
    const turnosPaciente = await this.pacienteService.obtenerTurnosPorEstado(
      pacienteId,
      estado,
    );
    return turnosPaciente.filter(
      (t) => String(t.medico._id ?? t.medico.id) === String(medicoId),
    );
  }
